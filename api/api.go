...
const (
	...
	// dummyRoflManifest is a minimal/dummy rofl.yaml used when no manifest is provided for validation.
	dummyRoflManifest = `name: validate-placeholder
version: 0.1.0
tee: tdx
kind: container
resources:
...
deployments:
  default:
    # Dummy ROFL app ID used for validation placeholder.
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/rofl", func(r chi.Router) {
			r.Post("/validate", func(w http.ResponseWriter, r *http.Request) {
				...
				// Use dummy rofl.yaml if manifest is not provided.
				manifest := req.Manifest
				if manifest == "" {
					manifest = dummyRoflManifest
				}
				...
				task, err := tasks.NewRoflValidateTask(addr, []byte(manifest), []byte(req.Compose))
				if err != nil {
